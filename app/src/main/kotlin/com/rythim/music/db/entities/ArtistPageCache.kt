...
import com.rythim.innertube.pages.ArtistSection
...
fun serializeArtistPage(
    ...
    artist: com.rythim.innertube.models.ArtistItem? = null,
): String {
...
fun CachedArtistPage.toArtistPage(): com.rythim.innertube.pages.ArtistPage {
    return com.rythim.innertube.pages.ArtistPage(
        artist = artist?.let { cached ->
            com.rythim.innertube.models.ArtistItem(
                id = cached.id,
                title = cached.title,
        ...
        sections = sections.map { section ->
            ArtistSection(
                ...
                moreEndpoint = section.moreBrowseId?.let { browseId ->
                    com.rythim.innertube.models.BrowseEndpoint(
                        browseId = browseId,
...
private fun CachedItem.toYTItem(): YTItem {
    return when (this) {
        is CachedItem.Song -> SongItem(
            ...
            thumbnail = thumbnail,
            explicit = explicit,
            artists = artists.map { com.rythim.innertube.models.Artist(it.name, it.id) },
            album = album?.let { com.rythim.innertube.models.Album(it.name, it.id) },
            duration = duration ?: 0,
            setVideoId = videoId,
        )
        is CachedItem.Album -> AlbumItem(
            ...
            thumbnail = thumbnail,
            explicit = explicit,
            artists = artists.map { com.rythim.innertube.models.Artist(it.name, it.id) }.ifEmpty { null },
            year = year,
        )
        is CachedItem.Playlist -> PlaylistItem(
            ...
            author = author?.let { com.rythim.innertube.models.Artist(it.name, it.id) },
        ...
        is CachedItem.Episode -> EpisodeItem(
            ...
            explicit = explicit,
            author = author?.let { com.rythim.innertube.models.Artist(it.name, it.id) },
            podcast = album?.let { com.rythim.innertube.models.Album(it.name, it.id) },
            duration = duration,
            endpoint = null,
        )
    }
